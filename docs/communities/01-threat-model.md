...
| Finding | Status |
| --- | --- |
| 002 — approval codes can be redirected | **Redirect FIXED v2.103.2**, **exchange binding FIXED v2.105.3**, **remainder open.** The callback is derived from the peer's registered base URL. The code exchange now compares the record against the *signing* peer rather than the `guestNode` in the body — a hostile peer could previously redeem an honest guest's code by naming it, taking the identity assertion and burning the single-use code. A refusal deliberately does not spend the code. **Still open:** pending requests are not bound to the initiating browser and the exchange does not check the stored `request_id`/`nonce` — a PKCE-shaped redesign, best done against a three-node lab. |
| 006 — unjoined peers inject into origin waves | **Closed in v2.104.1.** The origin-wave branch now requires the sender to be in that wave's `wave_federation` list, the same record the backfill endpoint has always consulted. When this node is the origin a peer may only attribute a message to its own namespace. A ping already cached is neither re-broadcast nor relayed, and relays are bounded by a per-wave budget. |
| 007 — remote message mutations unscoped | **Closed in v2.104.1.** Edits and deletions must name the wave the sender was authorized for, and the target ping must live in it; `markRemotePingDeleted` takes the wave. The upsert refuses to change a cached ping whose wave or origin differs, and the remote-user cache fallback is scoped to the node that owns the row. The same rule was applied to the legacy JSON store, where the conflict path replaced the whole record. |
...
| 009 — attachments served without authorization | **Closed in v2.104.0.** An `attachments` table maps each stored path to the wave it belongs to, and `/uploads` checks wave access on every request. Identity travels in a 15-minute HttpOnly cookie because an `<img>` cannot carry a header; a refusal is 404, not 403. Files with no wave — avatars, profile media, every upload predating the table — stay public: those URLs have already travelled, and withdrawing them would break existing conversations to close a gap that is already open. Encrypted waves are filed by the client after decrypt, since the server cannot read the message that would tell it. |
| 011 — restricted channels do not restrict | **FIXED v2.103.3.** A per-channel evaluator now reads `channel_permissions`. Restricted channels are hidden from the listing, refuse wave creation and filing, and can be opened to a role through new permission routes. Semantics are **admission, not enumeration**: being let in means your Community role governs what you do inside, with `deny` available to take one thing away. Staff who manage channels keep access deliberately. |
| 012 — remote invitation trusts a mutable role | **FIXED v2.105.0.** Binding re-checks `canInviteConferRole` against the role as it stands now, not as it stood at issuance; the invitee is admitted as an ordinary member and the withholding is audited. Remote invitations now expire (30 days) — they bind an address, not a person, and handles change hands. |
| 013 — resource ceilings and work budgets | **FIXED v2.105.0.** The member cap applies to bearer redemption and remote binding; the member list is paged with batched role lookups; the channel listing count is bounded and access-filtered; WebSocket frames are bounded by `maxPayload` rather than checked after buffering. Accumulation of soft-deleted rows through create/delete loops stays bounded by the mutation budget rather than by retention — noted, not fixed. |
| 014 — private Community metadata leaks | **FIXED v2.105.0.** One discovery decision (`canDiscoverCommunity`) runs before the error is chosen, so gated routes no longer distinguish absent from forbidden; the wave mappers and the single-wave route withhold container ids from readers who cannot discover the container; `waveCount` counts only readable waves. **Two parts declined with reasons:** the slug 409 is an accepted oracle (globally unique human-readable addresses and non-enumerability cannot both hold), and withdrawn home-node standing does not retract knowledge a member already had. |
| 015 — account deletion bypasses Community | **FIXED v2.105.0.** `detachUserFromCommunities` performs what the disabled foreign keys declare, asserted by `PRAGMA foreign_key_check` returning zero violations where it previously reported five. A Community whose last owner deletes their account is suspended reversibly with an audit record, rather than promoting a successor who never asked. |
| 016 — audit records incomplete, not atomic | **PARTLY FIXED v2.105.0.** The missing records are added — channel updates, both invitation revocations, node-admin closure. Atomicity of mutation-plus-audit is **deferred**: it is a transaction wrapper across ~29 handlers and belongs in its own change rather than inside a release already moving this much authorization code. |
| 017 — expiration and optional-field validation | **FIXED v2.105.0.** Expiries normalised to UTC ISO or refused, and an expiry already past is a refusal rather than a lifetime; `maxUses` validated as a bounded integer instead of silently meaning unlimited; role permission lists, names and priorities refused rather than treated as a no-op. |
| 018 — disabling Communities leaves paths active | **FIXED v2.105.0.** `/api/waves` channel filing and cross-port invitation binding both honour the feature switch. Logins still succeed and invitations stay pending for when it is switched back on. |
| 019 — channel deletion leaves inconsistent containment | **FIXED v2.105.0.** Both container columns are cleared in one transaction, and unfiling is scoped to the channel named in the URL — filing rights in one channel used to detach a wave from another, with the audit record attributed to the wrong Community. |
| 020 — no Community event federation | **By design.** Single-host Communities with cross-port members; §2 decided this. |
| 021 — ownership transfer incomplete | **Open**, and a product decision as much as a gap — but the only finding in the register where a user becomes genuinely **stuck**. `community.transfer` exists as a capability and nothing consumes it; `canGrantRole` refuses to grant at a priority ≥ your own, so an owner cannot appoint another owner; `wouldLeaveNoOwner` stops the last owner leaving. Their only exit is deleting their account, which suspends the Community (v2.105.1). Separately, cross-port stubs hold an empty password hash and step-up only ever bcrypt-compares a local password, so a **remote owner can never step up** for a destructive operation on their own Community. Needs an atomic, step-up-protected transfer *with acceptance* — being handed ownership carries obligations, and a silent transfer is a way to dump a liability on somebody. |
| 022 — removal does not revoke wave access | **Deliberate.** Wave privacy is the content authority; a Community never conferred it. |
| 023 — dependency reachability | **Informational, reviewed 2026-09-24.** Communities adds no third-party package. `multer@2.3.0` falls inside GHSA-3pph-fpjx-jg34 (≥2.2.0 <2.4.0) but the advisory covers `diskStorage` and every upload here uses `memoryStorage`, so it is **not reachable**; bump to 2.4.0 on the next dependency pass to keep it that way. `fluent-ffmpeg@2.1.3` is deprecated and archived upstream — a maintenance dead end rather than a vulnerability. Zero `npm audit` findings is not an application security pass. |
| 024 — dormant actor resolution, missing scope constraints | **FIXED v2.105.3.** `resolveActor` matched remote handles with `LIKE 'handle%'`, so `alice` also matched `alice_admin` — a collision cross-port auth itself creates by suffixing a clashing handle with the home node's first label. Both legitimate spellings are now matched exactly and nothing else is. Role grants are scoped to their Community on **both** sides: `grantCommunityRole` refuses to create a cross-Community row (better never to hold the invalid state) and the role readers refuse to return one (live nodes may already hold it). Neither had a reachable path, which is why they were worth closing while that was still true. |
