...
- **Every public event page links up to the whole-server index.** A wave's event list and each individual event now carry "See all events on this server →". Someone arriving on a shared link to one event previously had no way to discover that anything else was published.
...
- **A server-wide event's back button led to a dead page.** It pointed at `/events/server`, which is not a page — `server` is a reserved slug, so the router looked for a wave with that slug and rendered "No event page found at this address." It now goes to the index. The button on a wave event also now names where it goes ("← Public Test events") rather than saying "All events", which is what the index link says.
...
- Feature-gate 403s carry a machine-readable `code: 'FEATURE_DISABLED'` alongside the human-readable message, and `fetchAPI` now preserves `status`, `code` and `feature` on the Error it throws, so callers can branch on a code instead of pattern-matching prose. `CrawlBar` prefers the code and falls back to the message for federated nodes running older builds.

### Known

- Six `AuthProvider` handlers (`login`, `completeMfaLogin`, `register`, `updateUser`, `getPendingPassword`, `clearPendingPassword`) are still recreated per render, and the context value is an inline object literal, so every consumer re-renders whenever `AuthProvider` does. This no longer causes refetches now that `fetchAPI` is stable, but it is worth cleaning up separately.
...
- **Reaction picker on mobile**: Emoji row is now a single horizontally scrollable strip instead of wrapping and getting clipped. Swipe left/right to browse all reactions; emoji buttons are larger on mobile for easier tapping.
- **Quote jumps to original message**: Clicking the `↩ Name: text` parent quote above a reply now smoothly scrolls the message list to the original ping and briefly highlights it in amber so it's easy to spot.
...
- **Wave header cleanup**: Removed participant/ping count stats line and the Wave Toolbar row (playback, collapse/expand toggle, mark all read). Playback and Mark All Read moved into the `⋮` menu; collapse/expand all were already there.
